...
class Detector(object):
    ...
    NON_LATIN_RE = re.compile(r'[^A-z]')
    LATIN_RE = re.compile(r'[A-z]')
    SYMBOLS_RE = re.compile(r'[^\u0000-\u0299\u1E00—\u1EFF]')
    ...
    def set_prior_map(self, prior_map):
        ...
        self.prior_map = numpy.array([0.0] * len(self.langlist))
    ...
    def cleaning_text(self):
        ...
        latin = self.LATIN_RE.findall(self.text)
        latin_count = len(latin)

        non_latin = self.SYMBOLS_RE.findall(self.text)
        non_latin_count = len(non_latin)

        if latin_count * 2 < non_latin_count:
            self.text = self.LATIN_RE.findall(self.text)
    ...
    def _detect_block(self):
        ...
        self.langprob = numpy.array([0.0] * len(self.langlist))
        ...
        for t in xrange(self.n_trial):
            ...
            while True:
                self._update_lang_prob(prob, self.random.choice(ngrams), weight)
                if i % 5 == 0:
    ...
    def _init_probability(self):
        ...
        if self.prior_map is not None:
            return numpy.array(self.prior_map)
        else:
            return numpy.array([1.0 / len(self.langlist)] * len(self.langlist))
        ...
        for ch in self.text:
            ...
            for n in RANGE:
                ...
                if len(ngram.grams) < n:
                    break
                w = ngram.grams[-n:]
                if w in self.word_lang_prob_map:
                    result.append(w)
        return result

    def _update_lang_prob(self, prob, word, weight):
        ...
        lang_prob_map = lang_prob_map + weight
        prob *= lang_prob_map
    ...
    def _word_prob_to_string(self, prob):
        ...
        indexes = numpy.argwhere(prob > 0.00001)
        probs = numpy.take(prob, indexes)
        strings = numpy.take(self.langlist, indexes)

        res_list = [' %s:%.5f' % (lang[0], p) for p, lang in zip(probs, strings)]
        res = ' '.join(res_list)
        return res
    ...
    def _normalize_prob(self, prob):
        ...
        '''
        prob[:] = numpy.array(prob) / numpy.sum(prob)
        maxp = numpy.amax(prob)
        return maxp
    ...
    def generate_unicode(self, symbols):
        symbol = symbols.group(0)
        st = hex(0x10000 + ord(symbol))[2:]
        while len(st) < 4:
            st = '0' + st
        return r'\u' + st[1:5]

    def _unicode_encode(self, word):
        res = re.sub(r'[^\u0000-\u0080]', self.generate_unicode, word)
        return res
